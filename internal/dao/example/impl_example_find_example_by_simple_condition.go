...
func (cls *Example) FindExampleBySimpleCondition(ctx context.Context, field4 *model.ExampleEnumFieldType, page int, size int) (int64, []*model.Example, error) {
	...
	if err := session.Count(&total).Error; nil != err {
		...
		return 0, nil, errors.ErrorServerInternalError
	}
