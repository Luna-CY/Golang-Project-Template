...
type Example interface {
	...
	SaveExample(ctx icontext.Context, example *model.Example) error

	// TakeExampleById get example by id from db
	// if example not found, return ierror.ErrorRecordNotFound error
	TakeExampleById(ctx icontext.Context, id uint64, lock bool) (*model.Example, error)
	...
	FindExampleBySimpleCondition(ctx icontext.Context, field4 *model.ExampleEnumFieldType, page int, size int) (int64, []*model.Example, error)
}
